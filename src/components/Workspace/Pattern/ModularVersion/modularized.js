...
import patternHandlers from './patternHandlers'

export default class Pattern extends Component {
    constructor(props){
        super(props)
        const {handleVolumeChange} = patternHandlers(this)
        this.handleVolumeChange = handleVolumeChange
    }
    state={
        isSet: false,
        isLoaded: false,
    ...
    initialize=()=>{
        ...
        this.inst="C-1"
        this.playhead=(time)=> {
            let step = this.index % this.props.pattern.pattern.length;
            for (let i = 0; i < this.props.pattern.pattern.length; i++) {          
            if (this.props.pattern.pattern[step]) this.sampler.triggerAttackRelease(this.inst, 0.2, time);
            }
            this.index++;
        }
        ...
        this.loop = new Tone.Loop((time) => {
            this.playhead(time)
        }, `${this.props.pattern.pattern.length}n`).start()
    }
    ...
    render() {
        ...
        return (
            <div className="pattern">
                ...
                <button onClick={this.props.testFunc}>TESTFUNC</button>
                {this.renderPattern()}
            </div>
        )
    }
